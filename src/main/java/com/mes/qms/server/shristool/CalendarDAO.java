package com.mes.qms.server.shristool;
...
public class CalendarDAO {
	...
	// wSQLText = this.DMLChange(wSQLText);List<Map<String, Object>>
	// wQueryResultList = nameJdbcTemplate.queryForList(wSQLText, wParms);for
	// (Map<String, Object> wSqlDataReader : wQueryResultList) {
	// wSQLText = this.DMLChange(wSQLText);KeyHolder keyHolder = new
	// GeneratedKeyHolder();SqlParameterSource wSqlParameterSource = new
	// MapSqlParameterSource(wParms);nameJdbcTemplate.update(wSQLText,
	// wSqlParameterSource, keyHolder);wID = keyHolder.getKey().intValue();
}
